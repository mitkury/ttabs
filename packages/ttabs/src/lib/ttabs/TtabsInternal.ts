 ...
 */
export type StateChangeCallback = (state: TtabsState) => void;

/**
 * Interface for the Ttabs state that will be passed to subscribers
 */
...
/**
 * Ttabs core class implementation - framework agnostic
 */
export class TtabsInternal {
  // Base state properties
  private _tiles: Record<string, Tile> = {};
  private _activePanel: string | null = null;
  private _focusedActiveTab: string | null = null;
  
  // Root grid tracking
  private _rootGridId: string | null = null;
  
  // Theme state
  private _theme: TtabsTheme = DEFAULT_THEME;
  ...
  constructor(options: TtabsOptions = {}) {
    ...
    if (options.tiles) {
      if (Array.isArray(options.tiles)) {
        ...
        this._rootGridId = Object.values(this._tiles)
    ...
    if (options.focusedTab && this.getTile(options.focusedTab)) {
      const tab = this.getTile(options.focusedTab);
      if (tab && tab.type === 'tab') {
        this._focusedActiveTab = options.focusedTab;
      }
    }
  }
   ...
   */
  protected getState(): TtabsState {
    return {
      tiles: { ...this._tiles },
      activePanel: this._activePanel,
      focusedActiveTab: this._focusedActiveTab,
      rootGridId: this._rootGridId,
      theme: { ...this._theme }
    };
  }
  ...
  getTile<T extends Tile = Tile>(id: string): T | null {
    return (this._tiles[id] || null) as T | null;
  }
  ...
  getActivePanelTile(): TilePanel | null {
    return this._activePanel ? this.getTile<TilePanel>(this._activePanel) : null;
  }
  ...
  private findTilesReferencingTile(tileId: string): Tile[] {
    return Object.values(this._tiles).filter(tile => {
  ...
  getFocusedActiveTabTile(): TileTab | null {
    return this._focusedActiveTab ? this.getTile<TileTab>(this._focusedActiveTab) : null;
  }
  ...
  cleanupContainers(tileId: string): void {
    ...
    switch (tile.type) {
      ...
      case 'column':
        ...
        shouldRemove = !column.child || !this._tiles[column.child];
  ...
  resetState(): void {
    ...
    this._focusedActiveTab = null;
    this._rootGridId = null;
    this.notifyStateChange();
  }
  ...
  serializeLayout(): string {
    ...
    const tilesArray = Object.values(this._tiles);
        ...
        if (parsed && typeof parsed === 'object' && 'tiles' in parsed && Array.isArray(parsed.tiles)) {
          ...
          if (parsed.metadata && 'focusedActiveTab' in parsed.metadata) {
            ...
            if (focusedTabId && this.getTile<TileTab>(focusedTabId)) {
              this._focusedActiveTab = focusedTabId;
            }
          }
  ...
  private findAndSetDefaultFocusedTab(): void {
    ...
    if (this._activePanel) {
      const panel = this.getTile<TilePanel>(this._activePanel);
      if (panel && panel.activeTab) {
        this._focusedActiveTab = panel.activeTab;
      }
    }
    
    // Otherwise find the first panel with an active tab
    const panelsWithActiveTabs = Object.values(this._tiles)
    ...
    if (panelsWithActiveTabs.length > 0 && panelsWithActiveTabs[0].activeTab) {
      this._focusedActiveTab = panelsWithActiveTabs[0].activeTab;
    }
  }
  ...
  setTheme(theme: TtabsTheme): void {
    ...
    console.log('Theme updated:', this._theme.name);
  }

  /**
   * Check if a component is registered - should be overridden in framework-specific implementation
   * @param componentId The component identifier
   * @returns Always false in core implementation
   */
  protected hasContentComponent(componentId: string): boolean {
    return false;
  }
  ...
  createDefaultLayout(): string {
    ...
    const rootId = this._rootGridId as string;
  ...
  closeTab(tabId: string): boolean {
    try {
      ...
      if (this._focusedActiveTab === tabId) {
        ...
        if (panel.tabs.length > 1) {
          ...
          const otherPanels = Object.values(this._tiles)
            .filter((tile): tile is TilePanel => 
      ...
      if (newFocusedTab) {
        this._focusedActiveTab = newFocusedTab;
      }
